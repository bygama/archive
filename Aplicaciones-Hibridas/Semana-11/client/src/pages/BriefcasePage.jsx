...
export default function BriefcasePage({ user, onLogout, onRequestAuth }) {
  ...
  // modo invitado: si no hay usuario logueado el maletin no usa la api protegida,
  // sino que vive en localStorage. en los dos casos la respuesta tiene la misma
  // forma { caseSize, placed } para que el resto del componente no cambie.
  const isGuest = !user;
  ...
  const store = useMemo(() => {
    if (!isGuest) {
      return {
        get:    ()           => api.getBriefcase(),
        resize: (w, h)       => api.resizeBriefcase(w, h),
        add:    (pl)         => api.addItem(pl),
        move:   (idx, patch) => api.moveItem(idx, patch),
        remove: (idx)        => api.removeItem(idx),
        clear:  ()           => api.clearBriefcase(),
        save:   (state)      => api.saveBriefcase(state),
      };
    }
    const GUEST_KEY = 'maletin_guest_bc';
    const persist = (next) => {
    ...
    return {
      get: async () => {
        try {
          const s = JSON.parse(localStorage.getItem(GUEST_KEY));
          if (s && Array.isArray(s.placed)) return s;
        } catch {}
        return { caseSize: bcRef.current.caseSize, placed: [] };
      },
      resize: async (w, h)       => persist({ caseSize: { w, h }, placed: bcRef.current.placed }),
      add:    async (pl)         => persist({ caseSize: bcRef.current.caseSize, placed: [...bcRef.current.placed, pl] }),
      move:   async (idx, patch) => persist({ caseSize: bcRef.current.caseSize, placed: bcRef.current.placed.map((p, i) => i === idx ? { ...p, ...patch } : p) }),
      remove: async (idx)        => persist({ caseSize: bcRef.current.caseSize, placed: bcRef.current.placed.filter((_, i) => i !== idx) }),
      clear:  async ()           => persist({ caseSize: bcRef.current.caseSize, placed: [] }),
    ...
    (async () => {
      try {
        ...
        if (mine.caseSize.w !== gd.cols || mine.caseSize.h !== gd.rows) {
          bcState = await store.resize(gd.cols, gd.rows);
        }
        setBc({
        ...
        if (fitsAt(bc.placed, indexById, candidate, bc.caseSize, drag.idx ?? -1)) {
          try {
            if (drag.source === 'catalog') {
              const updated = await store.add({ itemId: drag.itemId, x: cell.x, y: cell.y, rot: drag.rot, count: 1 });
              setBc({ caseSize: updated.caseSize, placed: updated.placed });
            } else {
              const updated = await store.move(drag.idx, { x: cell.x, y: cell.y, rot: drag.rot });
              setBc({ caseSize: updated.caseSize, placed: updated.placed });
            }
            dropped = true;
          } catch (err) {
  ...
  const rotateInPlace = async (idx, p) => {
    ...
    try {
      const updated = await store.move(idx, { rot: newRot });
      setBc({ caseSize: updated.caseSize, placed: updated.placed });
  ...
  const removeAt = async (idx) => {
    try {
      const updated = await store.remove(idx);
      setBc({ caseSize: updated.caseSize, placed: updated.placed });
    } catch (err) { setError(err.message); }
  };
  ...
  const clearAll = async () => {
    ...
    try {
      const updated = await store.clear();
      setBc({ caseSize: updated.caseSize, placed: updated.placed });
    } catch (err) { setError(err.message); }
  };
  ...
  const autoSort = async () => {
    ...
    try {
      const updated = await store.save({ caseSize: cs, placed });
      setBc({ caseSize: updated.caseSize, placed: updated.placed });
    } catch (err) { setError(err.message); }
  };
      ...
      <header className="topbar">
        ...
        <div className="topbar-controls">
          ...
          {user ? (
            ...
            <>
              <span className="user-tag">invitado</span>
              <button onClick={onRequestAuth}>Iniciar sesión</button>
            </>
          )}
        </div>
      </header>
